...
export class CountdownComponent implements  OnInit{
  ...
  @Output() reached: EventEmitter<any> = new EventEmitter();
  display: any = [];
  displayNumbers: any = [];
  wasReached: Boolean = false;
  baseValue = [31, 24, 60, 60];
  colors = ['warn', 'accent', 'primary', 'warn'];
  ...
  _displayString() {
    ...
    if ((dateDifference < 100 && dateDifference > 0) || dateDifference < 0 && !this.wasReached) {
      this.wasReached = !this.enableDynamic;
      this.reached.next(this.currentEvent);
    }
